...
(defprotocol dh_sig diffie-hellman
  (defrole group-init
    ...
    (conf group-dist))
  (defrole init
    (vars (x rndx) (y expt) (alpha expt) (group na nb text) (a b name) (group-dist chan))
    (trace
     (recv group-dist (cat "Group id" group (exp (gen) alpha)))
     (send (enc (exp (exp (gen) alpha) x) (privk a)))
     (recv (enc a  (exp (exp (gen) alpha) y) (exp (exp (gen) alpha) x) (privk b)))
     (send (enc "final" b na (exp (exp (gen) alpha) y) (exp (exp (gen) alpha) x) (privk a)))
     (recv (enc na nb (exp (exp (exp (gen) alpha) y) x)))
     (send nb))
    (uniq-gen na x)
    (auth group-dist))
  (defrole resp
    (vars (y rndx) (x expt) (alpha expt) (group na nb text) (a b name) (group-dist chan))
    (trace
     (recv group-dist (cat "Group id" group (exp (gen) alpha)))
     (recv (enc (exp (exp (gen) alpha) x) (privk a)))
     (send (enc a (exp (exp (gen) alpha) y) (exp (exp (gen) alpha) x) (privk b)))
     (recv (enc "final" b na (exp (exp (gen) alpha) y) (exp (exp (gen) alpha) x) (privk a)))
     (send (enc na nb (exp (exp (exp (gen) alpha) x) y)))
     (recv nb))
    (uniq-gen nb y)
    (auth group-dist))
)
...
(defprotocol dh_sig2 diffie-hellman
  ...
  (defrole init
    ...
    (trace
     ...
     (send (enc (exp (exp (gen) alpha) x) (privk a)))
     (recv (enc a (exp (exp (gen) alpha) y) (exp (exp (gen) alpha) x) (privk b)))
     (send (enc n b (exp (exp (gen) alpha) (mul x y))))
     (recv n))
    (uniq-gen x n)
    (auth group-dist))
  (defrole resp
    ...
    (trace
     ...
     (recv (enc (exp (exp (gen) alpha) x) (privk a)))
     (send (enc a (exp (exp (gen) alpha) y) (exp (exp (gen) alpha) x) (privk b)))
     (recv (enc n b (exp (exp (gen) alpha) (mul x y))))
     (send n))
    (uniq-gen y)
    (auth group-dist))
)
