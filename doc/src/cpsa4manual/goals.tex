...
The remainder of this chapter is devoted to explaining the syntax
and semantics of this feature in greater detail.

\section{Syntax}\label{sec:goalsyntax}
...
\\ gvdecl&$\leftarrow$&(id+ sort)~$\mid$~(id+ \sym{strd})
...
\\ &$\mid$&(\sym{prec} strdvar integer strdvar integer)
\\ &$\mid$&(\sym{leads-to} strdvar integer strdvar integer)
\\ &$\mid$&(\sym{non} term)~$\mid$~(\sym{pnon} term)
\\ &$\mid$&(\sym{uniq} term)~$\mid$~(\sym{uniq-at} term strdvar integer)
\\ &$\mid$&(\sym{ugen} term)~$\mid$~(\sym{ugen-at} term strdvar integer)
...
exception, there is a new sort symbol \sym{strd}.  Notice that in the
sentence, variables \sym{z0} and \sym{z1} have sort \sym{strd}.  Every
...
\begin{center}
  \begin{tabular}{lcl}
    ...
    \sym{prec}& $\dom{strd}\times\dom{nat}\times\dom{strd}\times\dom{nat}$
                  & \mbox{Precedes} \\
    \sym{leads-to}& $\dom{strd}\times\dom{nat}\times\dom{strd}\times\dom{nat}$
                  & \mbox{Leads to} \\
    \sym{non}& \dom{atom}
    ...
    \sym{uniq-at}& $\dom{atom}\times\dom{strd}\times\dom{nat}$
                  & \mbox{Unique origination at a node} \\
    \sym{ugen}& \dom{atom}
                  & \mbox{Unique generation} \\
    \sym{ugen-at}& $\dom{atom}\times\dom{strd}\times\dom{nat}$
                  & \mbox{Unique generation on strand} \\
    $=$ & $\dom{all}\times\dom{all}$ & Equality \\
    \sym{fact} & $\dom{id}\times\dom{all}*$ & User defined fact
                                              predicating \dom{id}
    %   \\
%       \sym{absent} & $\dom{atom}\times\dom{mesg}$ & Random exponent
%                                                     absence 
  \end{tabular}
\end{center}
...
\index{goals!predicate symbols}
\index{p@\texttt{p} (goal predicate symbol)}
\ttindex{prec}
\index{leads-to@\texttt{leads-to} (goal predicate symbol)}
\index{non@\texttt{non} (goal predicate symbol)}
\index{pnon@\texttt{pnon} (goal predicate symbol)}
\index{uniq@\texttt{uniq} (goal predicate symbol)}
\index{uniq-at@\texttt{uniq-at} (goal predicate symbol)}
\index{ugen@\texttt{ugen} (goal predicate symbol)}
\index{ugen-at@\texttt{ugen-at} (goal predicate symbol)}
\index{fact@\texttt{fact} (goal predicate symbol)}
%\index{absent@\texttt{absent} (goal predicate symbol)}


\index{=@\texttt{=} (goal predicate symbol)}
...
\paragraph{``Assume'' declarations.}  A formula may be provided after
the \verb|trace| declaration in a \verb|defprotocol| declaration in an
\verb|assume| clause.  For instance:\footnote{See the file
  \texttt{dhcr\_um\_expt\_assume.scm} in the manual examples
  directory.}
%
...
\begin{description}
  ...
  \item[rely,] used after a reception to declare a formula containing
  only free variables appearing up to and including this reception.
  Every instance of the role long enough to include a successful
  instance of this reception must satisfy the formula; 
  \item[guar,] used before a transmission to declare a formula
  containing only free variables appearing up to and including the
  following transmission.  Every instance of the role long enough to
  include an instance of the following transmission must satisfy the
  formula.
\end{description}
%
