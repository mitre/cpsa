...
message algebra with \dom{strd}, the sort of strands and sort
\dom{indx} for natural numbers.  There is a zero constant and a
successor function for the natural numbers, however, we write those
terms using the usual numerals.

Security goals make use of protocol specific and protocol independent
predicates.  For each role $r\in P$, there is a protocol specific
binary strand length predicate $P[r]:\dom{strd}\times\dom{indx}$.  For
each role $\role_Y(C,N,U)\in P$ and variable~$x:S$ that occurs in~$C$,
there is a protocol specific binary strand parameter predicate
$P[r,x]:\dom{strd}\times{}S$.  The remaining predicates are protocol
independent.  The unary predicates are $\mathsf{non}: B$ and
$\mathsf{uniq}: B$ for each base sort~$B$.  The binary predicate is
equality.  The ternary predicate is $\mathsf{\uniqat}:
B\times\dom{strd}\times\dom{indx}$.  The quaternary predicate is
$\mathsf{prec}:\dom{node}\times\dom{indx}\times\dom{node}\times\dom{indx}$.
...
sort~\dom{strd} for each strand in~$k$.  Let $Z(s)$ be the strand
...
\item For each $s\in\sdom(I)$, let $I(s)=\cn{i}(r,h,\sigma)$ and
  $z=Z(s)$.  Assert $P[r](z,h)$.  Let $r=\role(C,N,U)$.  For each
...
z_0\colon\srt{Z}}(\\
\quad\resp(z_0,2)\wedge\resp_{a}(z_0,a_0)\wedge
\resp_{b}(z_0,b_0)\\
\qquad{}\wedge
...
d_0=d_1\\
\qquad\quad\wedge\resp(z_1,2)\wedge
\resp_{a}(z_1,a_1)\wedge
\resp_{b}(z_1,b_1)\\
\qquad\quad\wedge
\resp_{s}(z_1,s_1)\wedge
\resp_{d}(z_1,d_1)\\
\qquad\quad\wedge\init(z_2,1)\wedge\init_a(z_2,a_1)\wedge
...
Simple Example Protocol in Section~\ref{sec:blanchet's simple example
  ...
  sentence}.  The sort \dom{strd} is abbreviated as~\srt{Z}, the
strand length predicate $P[r](z,h)$ is written $r(z,h)$, and the
strand parameter predicate $P[r,x](z,t)$ is written $r_x(z,t)$ with the
protocol left implicit.

\paragraph{Semantics of Skeleton Formulas.}

Let $k=\skel_X(P,I,\prec,N,U)$ and $\Theta_X$ be its strand space.
The universe of discourse is $\alg{D}=\nat\cup\alga_X$.  When
formula~$\Phi$ is satisfied in skeleton~$k$ with variable assignment
$\alpha\colon Y\to \alg{D}$, we write $k,\alpha\models\Phi$.  We
write~$\bar\alpha$ when~$\alpha$ is extended to terms in the obvious
way.  When sentence~$\Gamma$ is satisfied in skeleton~$k$, we write
$k\models\Gamma$.

\begin{itemize}
\item $k,\alpha\models P[r](y, z)$ iff $\alpha(y)\in\sdom(\Theta_X)$,
  $\alpha(z)\in\nat$,
  $\alpha(y)=s$, $\alpha(z)=h$, and for some~$\sigma$,
  \[\prefix{\Theta_X(s)}{h}=\sigma\circ\prefix{C}{h}\]
  when $r=\role(C,N,U)$ and $1\leq h\leq|C|$.

\item $k,\alpha\models P[r,x](y,t)$ iff
  $\alpha(y)\in\sdom(\Theta_X)$, $\bar\alpha(t)\in\alga_X$, and with
  $\alpha(y)=s$ and for some~$\sigma$ with $\sigma(x)=\bar\alpha(t)$,
  \[\prefix{\Theta_X(s)}{i+1}=\sigma\circ\prefix{C}{i+1}\]
  when $r=\role(C,N,U)$ and $i$ is the position at which $x$ first
  occurs in $C$.
\end{itemize}
...
\item $k,\alpha\models\cn{\uniqat}(t,y,z)$ iff $\bar\alpha(t)\in U$ and
  $(\alpha(y),\alpha(z))\in\orig_k(\bar\alpha(t))$.
...
of role~$\rho$'s trace, there are $|\rho|$ unary length predicate
symbols $L_\rho^\ell$, with $0<\ell\leq|\rho|$.  Formula
$L_\rho^\ell(s)$ asserts about skeleton~$k$ that $s$ is a strand of
length at least~$\ell$ and it is an instance of role~$\rho$ after
truncating its length to~$\ell$.

Each algebra variable that occurs in a role is called a parameter.
Let $o(\rho,x)$ be the length of the smallest prefix of the role's
trace that contains~$x$.  There is a binary predicate symbol
$P_\rho^x$ for each parameter~$x$ in~$\rho$.  Formula $P_\rho^x(s,t)$
asserts about skeleton~$k$ that
\begin{enumerate}
\item $s$ is a strand of length at least~$o(\rho,x)$;
\item $s$ is an instance of role~$\rho$ after truncating its length
  to~$o(\rho,x)$; and
\item the strand instantiates $x$ as~$t$.
\end{enumerate}

The remaining predicate symbols are the same for all protocols.
Formula $\cn{prec}(u,v)$ asserts that~$u$ and~$v$ are nodes of
skeleton~$k$ and $u\prec v$.  In practice, since the rule terms must
denote nodes, uses of the $\cn{prec}$ symbol always have the form
$\cn{prec}(n_i(s),n_j(t))$, where $s$ and $t$ are variables.  Formulas
$\cn{non}(t)$, $\cn{pnon}(t)$, and $\cn{uniq}(t)$, assert $t$ is
non-originating in~$k$, penetrator non-originating in~$k$, and
uniquely originating in~$t$ respectively.  Formula
$\cn{fact}(\fn{name},t_1,t_2,\ldots,t_n)$ asserts that
$(\fn{name},\seq{t_1,t_2,\ldots,t_n})$ is a fact in~$k$.  Finally, $t_1
= t_2$ asserts equality.
\fi
...
\item $k,\alpha\models P[r,x](y,t)$ iff
  ...
  \[\prefix{\Theta_X(s)}{\ell}=\sigma\circ\prefix{C}{\ell}\]
  when $r=\role(C,N,U)$ and $\ell$ is the smallest $h$ such that
  $\prefix{C}{h}$ contains~$x$.
\end{itemize}
