...
Let $k=\skel_X(P,I,\prec,N,U)$.  The universe of discourse is
$\alg{D}=\nat\cup\alga_X$.  When formula~$\Phi$ is satisfied in
skeleton~$k$ with variable assignment $\alpha\colon Y\to \alg{D}$, we
write $k,\alpha\models\Phi$.  We write~$\bar\alpha$ when~$\alpha$ is
...
\item $k,\alpha\models\cn{\uniqat}(t,y,z)$ iff $\bar\alpha(t)\in\upsilon$ and
