...
The input may optionally start with a \index{herald
  form}\texttt{herald} form.  The form contains a title for the run
...
variable of sort \dom{chan}.  A channel may be \index{authenticated
  channel}authenticated or \index{confidential channel}confidential or
...
A channel is \index{authenticated channel}\emph{authenticated} if
only regular strands transmit messages on it.
A channel is \index{confidential channel}\emph{confidential} if
...
selected, called the \index{test node}\emph{test node.}  A term
carried by the inbound message at the test node is identified as the
\index{critical term}\emph{critical term.}  A critical term is one
...
\index{goal, security}\index{security goal}security goal. A
...
Each protocol may include a small collection of \index{rule}rules.  A
...
Algebra extensions are declared using a \texttt{lang} field in the
herald form of a source file.  For the certificate body example,
\begin{quote}
\begin{verbatim}
(herald prot (lang (cert-body (tupl 3))))
\end{verbatim}
\end{quote}
...
\end{verbatim}
\end{quote}
{\cpsa} represents this form internally as sequence of messages with a
...
\begin{tabular}{rcl}
  lang&$\leftarrow$&(\sym{lang} decl$\ast$)
  \\ decl&$\leftarrow$&(symbol+ kind)
  \\ kind&$\leftarrow$&
...
The syntax of the \index{lang field}\texttt{lang} field is given in
Table~\ref{tab:lang field syntax}.  There are eight kinds of ways that
{\cpsa} algebras can be extended.
...
\item An atomic sort is added to the algebra when a symbol is declared
  ...
  an atomic sort, than \verb|(price dollar)| in a \texttt{var} form
