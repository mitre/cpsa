...
Skeleton~$k_0$ \emph{reduces to} skeleton~$k_1$ by \emph{pruning},
written $k_0\reduction{\ops{P}_s}k_1$, iff there is a most general
unifier~$\sigma$ such that 
%
\begin{enumerate}
  \item $\sigma(\Theta_{k_0}(s)(j))\equiv \Theta_{k_0}(s')(j)$ for all
  $0\leq j<|\Theta_{k_0}(s)|$;
  \item no variable in $\sdom(\sigma)$ occurs in the trace of any
  strand other than~$s$;
  \item $\sigma$ is a renaming;
  \item $t\in U_{k_0}$ implies $\sigma(t)\in U_{k_0}$;
  \item $t\in N_{k_0}$ implies $\sigma(t)\in N_{k_0}$;
  \item there is a~$k$ such that
  $k_0\reduction{\ops{S}_\sigma}k\reduction{\ops{C}_{s,s'}}k_1$;
  \item there is a homomorphism from~$k_0$ to~$k_1$;
  \item if $(s,p)\kprec{k_0}(s'',p'')$ then
  $(s',p)\kprec{k_0}(s'',p'')$; and
  \item if $(s'',p'')\kprec{k_0}(s,p)$ then
  $(s'',p'')\kprec{k_0}(s',p)$.  
\end{enumerate}
%
For the setwise pruning reduction,
