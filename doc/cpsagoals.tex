...
The precise semantics of the goal language is in~\cite{cpsaspec09}.
The syntax of security goals appears in~\cite[Table~2]{cpsaprimer09}.
...
skeleton is the same.  Of course, an error is raised if the
role position formula is replaced by \texttt{(p "init" 1 z2)}.
...
originates.  Futhermore, the point of view asserts that nonce is
...
security goals are used in {\cpsa}.  However, there are more features
that may be useful.
...
from the first sentenc.  It is wise to ensure that the antecedent in
every sentence is identical.  If {\cpsa} tests goal
satisfaction\marginpar{Fix this!}
starting with a skeleton that is not the characteristic skeleton, one
cannot always extend the result to make a conclusion about the truth
of the goal in all executions.
...
Consider the first example in this paper (Figure~\ref{fig:ns init},
which is in the sample file \texttt{goals.scm}.  To generate a
maximally informative security goal from the initiator point of view
with \texttt{ghci} and \texttt{Make.hs}, type
\begin{quote}
\begin{verbatim}
$ ghci Make.hs
*Make> sas "goals"
