...
sameContractedEvent k (s, i) (s', i') =
    (s == s') &&
    (i == i' ||
     (nodeIsStateNode k (s, i) && nodeIsStateNode k (s', i') &&
      case compare i i' of
...
compressUpdate i j xs = 
  case (i < L.length xs, j < L.length xs) of 
    (True, True) ->
        map (updateStrand i j) xs        
--           error ("compressUpdate:  Cannot displace existing strand " ++
--                  (show i) ++ " onto existing strand " ++
--                  (show j))
    (True, False) ->
      let x = xs !! i in  
      L.delete x xs ++ [x]
    (_, _) -> xs
    -- Compressing a new strand onto an old or onto another new strand
    -- does not change strand map. 
